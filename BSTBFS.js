...
class BinarySearchTree {
    ...
    toArrLevelorder(current = this.root) {
        //new array to catch the visited nodes
        let arr = [];
        //new queue ensures nodes are added to array in level order
        let queue = new Queue();
        // put current in queue
        queue.enqueue(current);
        //after current is in queue
        // while check if current is not empty,
        while (!queue.isEmpty()) {
            //dequeue the current
            let dequeued = queue.dequeue();
            // push the data of the dequeued node into the array
